...
if __name__ == "__main__":
    ...
    s2 = scrapelib.Scraper(requests_per_minute=60)

    #startdate = date(2000, 1, 1)
    #startdate = date(2000, 4, 22)
    #startdate = date(2001, 4, 4)
    startdate = date(2017, 2, 4)
    ...
    while True:
        ...
        print soup.title
        try:
            for a in soup.find_all("div", {"class": "cnnSectBulletItems"}):
                l = a.find('a')
                url = l['href']
                ...
                try:
                    ...
                    #print soup2.find_all("div", {"class": "cnnBodyText"})
                    #data = extract_transcript(res2)
                    for p in soup2.find_all("p", {"class": "cnnBodyText"}):
                        print p.text
                        with open("texts.txt", "a") as myfile:
                            myfile.write(p.text)
                except Exception as e:
                    print e
